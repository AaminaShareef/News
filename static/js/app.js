...
function showSpinner(msg) {
    ...
    $('spinner-text').textContent = msg || 'Analysing…';
}
...
// Risk order for sorting (matches risk_aggregator.py levels)
var RISK_ORDER = { 'CRITICAL': 5, 'HIGH': 4, 'MEDIUM': 3, 'LOW': 1 };
...
function updateKPIs(data) {
    ...
    // CRITICAL count
    $('kpi-high').textContent = data.filter(function(d) {
        return (d.risk_level || '').trim() === 'CRITICAL';
    }).length;

    // HIGH count
    $('kpi-medium').textContent = data.filter(function(d) {
        return (d.risk_level || '').trim() === 'HIGH';
    }).length;

    // MEDIUM + LOW count
    $('kpi-low').textContent = data.filter(function(d) {
        var r = (d.risk_level || '').trim();
        return r === 'MEDIUM' || r === 'LOW';
    }).length;

    // Top industry — uses primary_industry or event_type or domain field
    var indCounts = {};
    data.forEach(function(d) {
        var k = _industry(d);
        if (k) indCounts[k] = (indCounts[k] || 0) + 1;
    });
    var entries = Object.entries(indCounts);
    $('kpi-event').textContent = entries.length
        ? entries.sort(function(a, b) { return b[1] - a[1]; })[0][0]
        : '—';
    ...
    if (el) {
        el.textContent = neg + ' / ' + pos;
        el.className   = 'kpi-value' + (neg > pos ? ' high' : neg < pos ? ' low' : ' medium');
    }
}
...
function checkHighRiskAlert(data) {
    ...
    var b = $('alert-banner');
    if (n > 0) { $('alert-count').textContent = n; b.classList.add('visible'); }
    else b.classList.remove('visible');
}

// ── Helper: extract industry label from a record ──────────────────────────────
// Supports both old field names (event_type, domain) and new (primary_industry, industry)
function _industry(d) {
    return (
        d.primary_industry ||
        d.industry         ||
        d.event_type       ||
...
// Helper: extract disruption type
function _disruption(d) {
    // disruption_types is an array in the new schema
...
function populateFilters(data) {
    var sets = {
        country:          new Set(),
        region:           new Set(),
        event_type:       new Set(),   // industry
        disruption_type:  new Set(),   // cause
        risk_level:       new Set(),
        sentiment:        new Set()
    };
    ...
    data.forEach(function(d) {
        ...
        var ind = _industry(d); if (ind) sets.event_type.add(ind);
        var dis = _disruption(d); if (dis) sets.disruption_type.add(dis);
    ...
    _buildFilterGroup('filter-disruption-type',
        Array.from(sets.disruption_type).sort(), 'disruption_type', data);
    _buildFilterGroup('filter-risk-level',
        ['CRITICAL','HIGH','MEDIUM','LOW'].filter(function(r) { return sets.risk_level.has(r); }),
        'risk_level', data);
...
function _buildFilterGroup(cid, options, field, data) {
    ...
    if (!options.length) {
        c.innerHTML = '<div style="font-size:0.73rem;padding:4px 2px;color:var(--text-muted)">—</div>';
        return;
    }
    options.forEach(function(opt) {
        var count = data.filter(function(d) {
            if (field === 'event_type')      return _industry(d)   === opt;
            if (field === 'disruption_type') return _disruption(d) === opt;
            return ((d[field] || '').trim()) === opt;
        }).length;
...
function applyFilters() {
    ...
    STATE.filtered = STATE.allResults.filter(function(d) {
        for (var field in active) {
            var val;
            if (field === 'event_type')      val = _industry(d);
            else if (field === 'disruption_type') val = _disruption(d);
            else val = (d[field] || '').trim();
...
function buildCard(d) {
    ...
    var ind    = _industry(d) || '—';
    var dis    = _disruption(d);
    var score  = parseFloat(d.risk_score) || 0;

    // Scale 0–5 risk score to 0–100%
    var pct = Math.min(Math.round((score / 5) * 100), 100);
    ...
    var secInds = d.secondary_industries || [];
    var secHtml = secInds.map(function(s) {
        return '<span class="badge badge-etype secondary">' + _esc(s.replace(/_/g, ' ')) + '</span>';
    }).join('');
    ...
    el.innerHTML =
        ...
        '</div>' +
        '<div class="card-meta">' +
            '<span class="badge badge-etype">' + _esc(ind) + '</span>' +
            secHtml +
            (dis ? '<span class="badge badge-cause">' + _esc(dis) + '</span>' : '') +
        ...
        '<div class="score-row">' +
            ...
            '<div class="score-bar-wrap">' +
                '<div class="score-bar-fill ' + rl + '" style="width:' + pct + '%"></div>' +
            '</div>' +
...
function buildPagination(totalPages) {
    var pg = $('pagination'); pg.innerHTML = '';
    if (totalPages <= 1) return;

    var prev = document.createElement('button');
    prev.className = 'page-btn'; prev.innerHTML = '&lsaquo; Prev';
    prev.disabled  = STATE.currentPage === 1;
    prev.onclick   = function() { STATE.currentPage--; renderPage(); };
    pg.appendChild(prev);

    var s = Math.max(1, STATE.currentPage - 3);
    ...
    next.className = 'page-btn'; next.innerHTML = 'Next &rsaquo;';
...
function renderCharts(data) {
    ...
    if (typeof renderDisruptionChart=== 'function') renderDisruptionChart(data);
    if (typeof renderMatrixChart    === 'function') renderMatrixChart(data);
...
async function runAnalysis() {
    ...
    hideDropdown();
    showSpinner('Fetching & analysing supply chain news…');
    try {
...
async function exportCSV() {
    ...
    try {
        ...
        a.href   = url;
        a.download = 'supply_chain_risk_' + new Date().toISOString().slice(0, 10) + '.csv';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $('close-alert').addEventListener('click', function() {
    ...
    // Dropdown item click → populate search + run
    ...
    $('news-grid').innerHTML =
        ...
        '<p>Choose an industry or disruption type from the search bar,<br>' +
        'or type your own query and click <strong>Analyse</strong>.</p>' +
        '</div>';
});