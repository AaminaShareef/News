 ...
 * Updated: domains → industries
 * Supports both old field names (event_type/domain) and new (primary_industry/industry)
 * Added: disruption cause chart, industry×cause matrix chart
 */
...
// ── Industry colours ──────────────────────────────────────────────────────────
var INDUSTRY_COLORS = {
    'Semiconductor & Electronics': '#b78cf7',
    'Automotive':                  '#38c8f0',
    'Pharmaceuticals & Healthcare':'#30d999',
    'Food & Agriculture':          '#ff9547',
    'Energy & Oil & Gas':          '#f06aad',
    'Retail & Consumer Goods':     '#ffd166',
    'Shipping & Logistics':        '#06d6a0',
    'Aerospace & Defense':         '#118ab2',
    'Construction & Raw Materials':'#ef476f',
    'Technology & IT Hardware':    '#a8dadc',
    // Legacy / fallback
    'Geopolitical': '#b78cf7', 'Logistics': '#38c8f0',
    'Weather':      '#30d999', 'Labour':    '#ff9547',
    'Economic':     '#f06aad',
};

// ── Disruption cause colours ──────────────────────────────────────────────────
var CAUSE_COLORS = {
    'conflict geopolitical':   '#ff3d54',
    'weather climate':         '#38c8f0',
    'labour strike':           '#ff9547',
    'economic commodity':      '#f5a623',
    'pandemic health':         '#b78cf7',
    'political policy':        '#06d6a0',
    'cyber attack':            '#ef476f',
    'regulatory compliance':   '#a8dadc',
};
...
var CHART_RISK_ORDER      = ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'MEDIUM-HIGH', 'LOW-MEDIUM'];
...
// ── Field helpers ─────────────────────────────────────────────────────────────
// Reads industry label from any supported field name
function _ind(d) {
    return (d.primary_industry || d.industry || d.event_type || d.domain || '').trim();
}
function _rl(d)   { return (d.risk_level || '').trim(); }
function _sent(d) { return (d.sentiment  || '').trim(); }
function _cause(d) {
    if (Array.isArray(d.disruption_types) && d.disruption_types.length) {
        return d.disruption_types[0].replace(/_/g, ' ');
    }
    return (d.disruption_type || '').trim();
}
function _indColor(label) { return INDUSTRY_COLORS[label] || '#94a3b8'; }
function _causeColor(label) { return CAUSE_COLORS[label] || '#94a3b8'; }

function _showEmpty(id, msg) {
    var el = document.getElementById(id); if (!el) return;
    el.style.display = 'none';
    var p = el.parentNode;
    var ex = p.querySelector('.chart-empty-msg'); if (ex) ex.remove();
...
function _clearEmpty(id) {
    var el = document.getElementById(id); if (!el) return;
    el.style.display = '';
    var ex = el.parentNode.querySelector('.chart-empty-msg'); if (ex) ex.remove();
}

// ── 1. Events by industry (bar) ───────────────────────────────────────────────
function renderBarChart(data) {
    _sd('bar'); _clearEmpty('bar-chart');
    var counts = {};
    data.forEach(function(d) { var k = _ind(d); if (k) counts[k] = (counts[k] || 0) + 1; });
    ...
    var colors = labels.map(_indColor);
    var ctx = document.getElementById('bar-chart').getContext('2d');
    _ch.bar = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [{
        ...
        options: {
            ...
            scales: {
                x: { grid: { color: G }, ticks: { color: K, maxRotation: 35, font: { size: 9 } }, border: { color: B } },
...
function renderPieChart(data) {
    ...
    if (!labels.length) labels = Object.keys(counts).filter(Boolean);
...
function renderLineChart(data) {
    ...
    _ch.line = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: { grid: { color: G }, ticks: { color: K, maxRotation: 35, font: { size: 9 } }, border: { color: B } },
                y: { grid: { color: G }, ticks: { color: K }, border: { color: B }, beginAtZero: true,
                     title: { display: true, text: 'Risk Score', color: K, font: { size: 9 } } },
            },
        },
    });
}

// ── 4. Avg risk by industry (horizontal bar) ──────────────────────────────────
function renderAvgRiskChart(data) {
    _sd('hbar'); _clearEmpty('hbar-chart');
    var sums = {}, cnts = {};
    data.forEach(function(d) {
        var k = _ind(d); if (!k) return;
    ...
    _ch.hbar = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [{
                label: 'Avg Risk', data: avgs,
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: Object.assign({}, TT, { callbacks: {
                    title: function(c) { return c[0].label; },
                    label: function(c) { return ['  Avg: ' + c.parsed.x.toFixed(3), '  ' + cnts[c.label] + ' article' + (cnts[c.label] !== 1 ? 's' : '')]; },
                }}),
            },
            scales: {
                x: { min: 0, max: 5, grid: { color: G }, ticks: { color: K, callback: function(v) { return v.toFixed(1); } },
                     border: { color: B }, title: { display: true, text: 'Avg Risk Score (0–5)', color: K, font: { size: 9 } } },
                y: { grid: { color: 'transparent' }, ticks: { color: '#ddeef8', font: { size: 9 } }, border: { color: B } },
            },
        },
    });
}

// ── 5. Risk mix per industry (stacked bar) ────────────────────────────────────
function renderStackedChart(data) {
    _sd('stacked'); _clearEmpty('stacked-chart');
    var industries = [];
    data.forEach(function(d) { var k = _ind(d); if (k && industries.indexOf(k) === -1) industries.push(k); });
    var risks = CHART_RISK_ORDER.filter(function(r) { return data.some(function(d) { return _rl(d) === r; }); });
    if (!industries.length) { _showEmpty('stacked-chart', 'No data'); return; }
    if (!risks.length) { data.forEach(function(d) { var r = _rl(d); if (r && risks.indexOf(r) === -1) risks.push(r); }); }
    if (!risks.length) { _showEmpty('stacked-chart', 'No risk levels'); return; }

    var matrix = {};
    risks.forEach(function(r) { matrix[r] = {}; industries.forEach(function(d) { matrix[r][d] = 0; }); });
    data.forEach(function(d) { var k = _ind(d), r = _rl(d); if (k && r && matrix[r]) matrix[r][k] = (matrix[r][k] || 0) + 1; });

    var datasets = risks.map(function(r) {
        return {
            label: r, data: industries.map(function(d) { return matrix[r][d] || 0; }),
    ...
    _ch.stacked = new Chart(ctx, {
        type: 'bar',
        data: { labels: industries, datasets: datasets },
        options: {
            responsive: true, maintainAspectRatio: false,
            interaction: { mode: 'index', intersect: false },
            plugins: {
            ...
            scales: {
                x: { stacked: true, grid: { color: G }, ticks: { color: K, maxRotation: 35, font: { size: 9 } }, border: { color: B } },
...
// ── 7. Events by disruption cause (NEW) ──────────────────────────────────────
function renderDisruptionChart(data) {
    _sd('disruption'); _clearEmpty('disruption-chart');
    var counts = {};
    data.forEach(function(d) {
        // Handle array of disruption_types
        var types = Array.isArray(d.disruption_types) ? d.disruption_types : [];
        if (!types.length && d.disruption_type) types = [d.disruption_type];
        types.forEach(function(t) {
            var k = (t || '').replace(/_/g, ' ').trim();
            if (k) counts[k] = (counts[k] || 0) + 1;
        });
    });
    var labels = Object.keys(counts).sort(function(a, b) { return counts[b] - counts[a]; });
    if (!labels.length) { _showEmpty('disruption-chart', 'No disruption cause data'); return; }
    var values = labels.map(function(l) { return counts[l]; });
    var colors = labels.map(_causeColor);
    var ctx = document.getElementById('disruption-chart').getContext('2d');
    _ch.disruption = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [{
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: Object.assign({}, TT, { callbacks: {
                    title: function(c) { return c[0].label; },
                    label: function(c) { return '  ' + c.parsed.y + ' event' + (c.parsed.y !== 1 ? 's' : ''); },
                }}),
            },
            scales: {
                x: { grid: { color: G }, ticks: { color: K, maxRotation: 35, font: { size: 9 } }, border: { color: B } },
                y: { grid: { color: G }, ticks: { color: K, stepSize: 1 }, border: { color: B }, beginAtZero: true,
                     title: { display: true, text: 'Events', color: K, font: { size: 9 } } },
            },
        },
    });
}

// ── 8. Industry × Cause matrix (NEW) ─────────────────────────────────────────
function renderMatrixChart(data) {
    _sd('matrix'); _clearEmpty('matrix-chart');
    var industries = [], causes = [];
    data.forEach(function(d) {
        var ind = _ind(d); if (ind && industries.indexOf(ind) === -1) industries.push(ind);
        var types = Array.isArray(d.disruption_types) ? d.disruption_types : [];
        if (!types.length && d.disruption_type) types = [d.disruption_type];
        types.forEach(function(t) {
            var k = (t || '').replace(/_/g, ' ').trim();
            if (k && causes.indexOf(k) === -1) causes.push(k);
        });
    });
    if (!industries.length || !causes.length) { _showEmpty('matrix-chart', 'No cross-tab data'); return; }

    // Build matrix[cause][industry]
    var matrix = {};
    causes.forEach(function(c) { matrix[c] = {}; industries.forEach(function(i) { matrix[c][i] = 0; }); });
    data.forEach(function(d) {
        var ind   = _ind(d); if (!ind) return;
        var types = Array.isArray(d.disruption_types) ? d.disruption_types : [];
        if (!types.length && d.disruption_type) types = [d.disruption_type];
        types.forEach(function(t) {
            var k = (t || '').replace(/_/g, ' ').trim();
            if (k && matrix[k]) matrix[k][ind] = (matrix[k][ind] || 0) + 1;
        });
    });

    var datasets = causes.map(function(c) {
        var col = _causeColor(c);
        return {
            label: c, data: industries.map(function(i) { return matrix[c][i] || 0; }),
            backgroundColor: col + '88', borderColor: col, borderWidth: 1,
            borderRadius: 3, borderSkipped: false,
        };
    });
    var ctx = document.getElementById('matrix-chart').getContext('2d');
    _ch.matrix = new Chart(ctx, {
        type: 'bar',
        data: { labels: industries, datasets: datasets },
        options: {
            responsive: true, maintainAspectRatio: false,
            ...
            scales: {
                x: { stacked: true, grid: { color: G }, ticks: { color: K, maxRotation: 35, font: { size: 9 } }, border: { color: B } },
                y: { stacked: true, grid: { color: G }, ticks: { color: K, stepSize: 1 }, border: { color: B }, beginAtZero: true,
                     title: { display: true, text: 'Count', color: K, font: { size: 9 } } },
            },
        },
    });
}

// ── 9. Sentiment per industry (stacked) ──────────────────────────────────────
function renderSentimentChart(data) {
    ...
    if (!industries.length || !sents.length) { _showEmpty('sentiment-chart', 'No sentiment data'); return; }

    var matrix = {};
    sents.forEach(function(s) { matrix[s] = {}; industries.forEach(function(i) { matrix[s][i] = 0; }); });
    data.forEach(function(d) { var k = _ind(d), s = _sent(d); if (k && s && matrix[s]) matrix[s][k] = (matrix[s][k] || 0) + 1; });

    var datasets = sents.map(function(s) {
        return {
            label: s, data: industries.map(function(i) { return matrix[s][i] || 0; }),
